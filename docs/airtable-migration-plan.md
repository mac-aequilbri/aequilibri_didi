...
- ⬜ **AI-generation services** read job context from Postgres: variation draft, weekly report (`src/services/platform/construction/{variations,reports}.ts` via `prisma.platJob.findFirst`). **Blocked by / entangled with the job-picker dependency below** — pointing these reads at Airtable is necessary but NOT sufficient: the pages that invoke them feed a numeric `jobId` from a Postgres-fed `<select>`, which is empty for an Airtable-only org. (Minutes *extraction* takes raw text + jobId and needs no job-context read; only its picker is affected.)
- ⬜ **Job-picker dependency (new finding, larger than P1 as first scoped).** ~15 pages build their job `<select>` from `prisma.platJob.findMany` and post a numeric `jobId`: `{actions,budget,cashflow,decisions,meeting-minutes,procurement,quotes,risks,variations}/new`, plus `reports`, `delay-cascade`, the dashboard `page.tsx`, `portal`, `search/route.ts`. For an Airtable-only org these are empty, so no domain record can be created against an Airtable job. Recommended fix: a shared `loadJobOptions(ctx)` source (reuse `jobsListSource` shape — id + name, id is `rec…` in Airtable mode), thread the picked `RecordId` through each page's action via `recordIdParam`, and migrate the two AI-context reads (`aiDraftVariation`, `generateWeeklyReport`) onto an Airtable job-context loader (jobDetailSource-style: phases/budget/risks/cashflow/variations filtered by `Job` link). This is its own phase.
- **Acceptance criterion (met for all four detail pages):** after accepting an assessment in Airtable mode, the job/variation/quote/minutes detail pages render from the Airtable base.
...
5. **AI-context reads + job-picker migration** (P1, bigger than first scoped): build a shared Airtable-aware job-picker source, thread `RecordId` job ids through the ~15 picker pages + their actions, and point `aiDraftVariation`/`generateWeeklyReport` at an Airtable job-context loader. See the P1 "Job-picker dependency" note.
