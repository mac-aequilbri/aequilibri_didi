...
- **#2 — PENDING (still hard Postgres deps even with #1):** `writeRecord` writes the **audit log** to Postgres (best-effort/dropped in Airtable mode), the **propose/confirm approval queue** (`PlatPendingWrite`) is Postgres-only (breaks the assistant's deferred writes without it), and `snapshotIntelligence` + the scheduler write **snapshot history** to Postgres. These need Airtable homes (`EXECUTION_LOG`, a `PENDING_WRITES` table, `INTELLIGENCE_SNAPSHOT`) + `writeRecord`/`executeProposal` rewrites before prod can truly run Postgres-free. Plus the group-C picker/display reads (documents/BIM/job-edit/dashboard/search/assistant).
