...
- **⚠️ REQUIRED before this works on a base:** run `node scripts/airtable-add-hypothesis-link.mjs <baseId>` on the **template base** (so new client bases clone the field) **and every already-provisioned client base** (they were cloned before the field existed). Needs a valid PAT.
