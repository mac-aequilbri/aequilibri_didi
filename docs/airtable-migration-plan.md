...
### P5 — Zero-Postgres control plane (the org registry / auth bootstrap)
Goal: **no Postgres in production.** The domain data is on Airtable, but the *bootstrap* layer (resolve slug→base, org settings, auth/team) was still Postgres. Split into #1 (registry/auth — DONE) and #2 (audit log + approval queue + snapshot writes — PENDING).
- **#1 — Control base — ✅ DONE.** A single shared **control base** holds the org registry. [control.ts](../src/lib/airtable/control.ts): `PLAT_ORG_REGISTRY` (slug → orgId, baseId, settings, engagement config, isActive) + `PLAT_TEAM` (orgSlug, email, role). Gated on **`AIRTABLE_CONTROL_BASE_ID`** (+ `AIRTABLE_MIGRATION`); unset → unchanged Postgres behaviour. Wired: `resolveBaseId` (config.ts), `getOrgCtx`/`findMember`/`getCurrentUser` (org-context.ts), the org picker (app/page.tsx), the scheduler org-list, and onboarding (control path allocates `orgId` from the registry, writes org+admin to the control base, mirrors cfg/rules into the client base, and **skips the Postgres txn entirely**). Auth model: Clerk stays the identity provider; the control base is the membership/role source.
  - **⚠️ REQUIRED:** create a control base, run `node scripts/airtable-create-control-base.mjs <controlBaseId>`, then set `AIRTABLE_CONTROL_BASE_ID` (local `.env` + Render). Existing Postgres orgs need their rows copied into the registry (one-off) before flipping it on.
- **#2 — IN PROGRESS.** Core write-path + universal reads now Postgres-free:
  - ✅ **Audit log → Airtable `EXECUTION_LOG`** (`writeExecutedLog`, best-effort; actor.type → `Initiated_By`).
  - ✅ **Snapshot → Airtable `INTELLIGENCE_SNAPSHOT`** (`snapshotIntelligence` write + prev read; rich metrics ride in `Accuracy_Summary` JSON; `learningSource` reads it back).
  - ✅ **`createJobWithCode`** skips the Postgres read in Airtable mode (acceptance no longer 500s).
  - ✅ **Layout nav counts** (`navCountsSource`) + **dashboard** (`dashboardSource`) read the base — so every page + the landing page load Postgres-free. (`pending`/approval count is 0 in Airtable mode.)
  - ⬜ **Still Postgres (deferred):** the **propose/confirm approval queue** (`PlatPendingWrite`) — assistant-only (`executor.ts`); needs a `PENDING_WRITES` table + `writeRecord`/`executeProposal` rewrite + the approvals page. The **AI assistant** (`chat.ts`/`executor.ts`, ~22 prisma calls). **Accounting** (`PlatConAccountingConnection` holds OAuth tokens — mapping doc says keep app-side/encrypted, NOT Airtable). And the remaining feature reads: `documents/*`, `projects/[id]/{models,edit}`, `search/route.ts`, `exec-log`, `approvals`, `reports/[id]`, `project-plan`, `risks/escalation`, `delay-cascade`, `portal` (portal intentional). These still need Postgres if visited; the core onboard→assess→accept→domain→learning flow does not.
