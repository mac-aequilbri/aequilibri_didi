...
- **Deferred (schema gap, not a decision):** the corrections→hypotheses **clustering** stays Postgres "engine state." The canonical Airtable `CORRECTIONS`/`HYPOTHESES` tables have **no Corrections→Hypotheses link field**, so the engine's `correction.hypothesisId` clustering can't be represented faithfully without a base **schema addition** (add the link via the meta API) + live verification. The loop machinery (corrections, hypotheses, snapshots) therefore remains Postgres episodic input that feeds rule promotion; `learningSource` reads those from Postgres in both modes. **This is the one remaining P2 item, and it needs a live base to build+verify.**
- **Not yet live-verified:** the local `AIRTABLE_PAT` is stale, so the rule/config round-trip was checked only via tsc/eslint/fieldMaps-test. Verify on Render: onboard an Airtable org → confirm categories/settings/seed-rules land in the base and the learning-rules page renders them.
...
6. ✅ **DONE — Learning/config reads** (P2, `922e3e7`+`f1424e8`): full migration — config tier + learning-rule lifecycle on Airtable; onboarding mirrors config + seed rules. Remaining P2 sub-item: the corrections→hypotheses clustering needs a base schema addition (Corrections→Hypotheses link) + live verification before it can leave Postgres.
