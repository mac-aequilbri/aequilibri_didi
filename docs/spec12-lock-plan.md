...
- **Activation checklist (ops)**: (1) run the schema-drift migration (admin `/schema-drift`) against templates + Didi + legal to provision Override_Level/Application_Window (+ L1's JOBS.Engagement_Type, ENGAGEMENT_TYPE_CONFIG.Portfolio_View); (2) on each existing org, click "Seed cascade rules" on `/learning-rules`; (3) owner activates CASCADE-D/F/G drafts when ready for auto-writes.
