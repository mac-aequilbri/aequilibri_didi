    ...
    <div className="bg-white min-h-screen text-neutral-900">
      <div className="max-w-3xl mx-auto p-8">
        ...
        <div className="grid grid-cols-2 gap-6 mb-6 text-sm">
          <div>
            <div className="text-xs uppercase tracking-wide text-neutral-500 mb-1">Quote for</div>
            <div className="font-semibold">{quote.clientName || "—"}</div>
            <div className="text-neutral-600">{quote.title}</div>
          </div>
          <div>
            <div className="text-xs uppercase tracking-wide text-neutral-500 mb-1">Project</div>
          ...
          <tbody>
            {quote.lines.map((l) => (
              <tr key={l.id} className="border-b border-neutral-100">
                <td className="py-2">
                  {l.description}
                  {l.category ? <span className="block text-xs text-neutral-500">{l.category}</span> : null}
                </td>
        ...
        {quote.notes && (
          <div className="text-sm border-t border-neutral-200 pt-4">
            <div className="text-xs uppercase tracking-wide text-neutral-500 mb-1">Notes &amp; terms</div>
