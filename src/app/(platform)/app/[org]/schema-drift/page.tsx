...
function MigrateResultBanner({ sp }: { sp: Record<string, string | string[] | undefined> }) {
  ...
  const map: Record<string, { variant: MessageVariant; text: string }> = {
    ok: { variant: "success", text: `Migrated ${migrated} — applied ${applied} schema change(s).` },
    partial: { variant: "warning", text: `Migrated ${migrated} with errors — applied ${applied} change(s). See the row for remaining drift.` },
    noop: { variant: "info", text: `${migrated} was already in sync — nothing to apply.` },
    unavailable: { variant: "info", text: "Migration unavailable (Airtable mode off or no base id)." },
    unknown_base: { variant: "danger", text: "Refused: that base id is not managed by this platform." },
  };
  const m = map[status];
  if (!m) return null;
  return (
...
function StatusBadge({ org }: { org: OrgDrift }) {
  if (!org.reachable) return <Chip variant="warning">unreachable</Chip>;
  if (org.inSync) return <Chip variant="success">in sync</Chip>;
  return <Chip variant="danger">drift</Chip>;
}
    ...
    <div className="p-6 max-w-4xl">
      ...
      <section className="ae-card p-5 mb-6 space-y-2 text-sm">
        <div className="flex justify-between">
          ...
          <Chip variant={report.enabled ? "success" : "neutral"} className="font-mono">
