      ...
      <div className="px-8">
        <div className="ae-card overflow-hidden">
          <table className="ae-table">
            <thead><tr><th>Report</th><th>Client</th><th>Grade</th><th>Urgency</th><th>Status</th><th scope="col" className="text-right">Price inc GST</th><th>Generated</th></tr></thead>
            <tbody>
              {rows.length === 0 ? (
                ...
                rows.map((r) => (
                  <tr key={r.id}>
                    <td className="font-semibold"><Link href={`/uc1/condition-reports/${r.id}`} className="text-ae-info hover:underline">{r.reportNumber}</Link></td><td>{r.clientName || "—"}</td>
