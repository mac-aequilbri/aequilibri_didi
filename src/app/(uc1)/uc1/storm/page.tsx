      ...
      <div className="px-8">
        <div className="ae-card overflow-hidden">
          <table className="ae-table">
            <thead><tr><th>Event</th><th>Type</th><th>State</th><th scope="col" className="text-right">Leads</th><th>Date</th></tr></thead>
            <tbody>
              {rows.length === 0 ? (
                ...
                rows.map((e) => (
                  <tr key={e.id}>
                    <td className="font-medium"><Link href={`/uc1/storm/${e.id}`} className="text-ae-info hover:underline">{e.name}</Link></td><td>{e.eventType}</td><td>{e.state}</td>
