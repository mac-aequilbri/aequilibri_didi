...
//
// Posture and guards:
//  · Airtable mode only (Airtable is the system of record; Postgres is the
//    legacy dev fallback).
...
async function cascadeProcurementToCashflow(ctx: OrgCtx, write: CascadeWrite): Promise<void> {
  ...
  const payload = {
    ...
    status,
    notes: `Auto-created by ${marker} (CASCADE-D). Do not remove the marker.`,
    ...(jobId ? { jobId } : {}),
  };
...
async function cascadeBlockerToPhaseRag(ctx: OrgCtx, write: CascadeWrite): Promise<void> {
  const phaseId = typeof write.data.phaseId === "string" ? write.data.phaseId : null;
  if (!phaseId?.startsWith("rec")) return;

  const phase = await core.get(ctx.orgSlug, "PHASES", phaseId);
  if (!phase) return;
  const rag = normalizeRag(phase["RAG"]);
  if (rag === "Red" || rag === "Amber") return; // already at/above the floor
...
async function cascadeRiskToIssue(ctx: OrgCtx, write: CascadeWrite): Promise<void> {
  const riskId = typeof write.recordId === "string" ? write.recordId : null;
  if (!riskId?.startsWith("rec")) return;
  const risk = await core.get(ctx.orgSlug, "RISKS", riskId);
  if (!risk) return;

  // Existing materialisation issue for this risk → nothing to do (idempotent
  // on the ISSUES.RISKS link the action field map writes from riskId).
  const issues = await core.list(ctx.orgSlug, "ISSUES", { maxRecords: 1000 });
  const already = issues.some(
    (i) => S(i["Issue_Type"]) === "Risk Materialised" && firstLink(i["RISKS"]) === riskId,
  );
  if (already) return;

  const jobId = firstLink(risk["Job"]);
  const { writeRecord } = await import("./recordWriter");
  await writeRecord(ctx, {
    table: "action",
    op: "create",
    data: {
      title: `Risk materialised: ${S(risk["Risk"]) || "(risk)"}`.slice(0, 300),
      detail: S(risk["Mitigation"]) ? `Planned mitigation: ${S(risk["Mitigation"])}` : "",
      priority: "P1",
 ...
 *  coordination queue surfaces it until dismissed. */
  ...
  await core.create(ctx.orgSlug, "EXECUTION_LOG", {
    ...
    Summary: JSON.stringify({
      cascade: {
        ...
        table: write.table,
        recordId: typeof write.recordId === "string" ? write.recordId : null,
      },
    }),
    Initiated_By: "System",
    Status: "Ongoing",
